...
async function carregarUsuario() {
  try {
    const { data, error } = await supabase
      .from('receitas')
      .select('id, descricao, avatar_base64')
    ...
    if (data) {
      ...
      if (data.avatar_base64) {
        userAvatar.src = data.avatar_base64;
      } else {
  ...
  if (receitas.length === 0) {
    tabelaContainer.innerHTML = `
      <div class="empty-state">
        ...
        <button class="nav-button" onclick="document.getElementById('btn-nova-receita').click()">Adicionar Primeira Receita</button>
      </div>
    `;
    return;
  }
  ...
  receitas.forEach(receita => {
    ...
    tableHTML += `
      <tr>
        ...
        <td><button class="action-btn delete-btn" onclick="deleteReceita(${receita.id})">Excluir</button></td>
      </tr>
    `;
  });
...
function montarGrafico() {
  ...
  if (currentChartType === 'pie') {
    datasets = [{
      ...
      backgroundColor: ['#009ee3','#00a650','#ff6b00','#ff3b30','#ff9500','#34c759','#0077b6','#00a650','#ff5252','#2ecc71']
    }];
  } else {
    datasets = [
...
async function handleSubmit(e) {
  ...
  try {
    const { data, error } = await supabase
      .from('receitas')
      .insert([{ mes: `${mesInput}-01`, salario, descontos, descricao }])
      .select();
